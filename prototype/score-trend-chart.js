...
(function () {
  ...
  function drawRecharts(el, cfg) {
    ...
    var w = Math.round(el.getBoundingClientRect().width) || el.offsetWidth || 600;
    var ht = cfg.height || 240;
    var chart = h(R.AreaChart, { width: w, height: ht, data: data, margin: { top: 10, right: 22, left: 4, bottom: 0 } }, children);
  ...
  // True only once Recharts has actually painted an <svg> into the element.
  function rechartsPainted(el) {
    return !!(el.querySelector && el.querySelector('svg.recharts-surface, svg'));
  }
  ...
  function boot(el) {
    var cfg = parse(el);
    // Native chart paints immediately so there's never an empty gap.
    drawNative(el, cfg);
    // Try to upgrade to Recharts; only KEEP it if an <svg> actually paints, otherwise
    // the native chart stays (createRoot().render() not throwing is NOT proof of a render).
    var tries = 0;
    var attempt = function () {
      if (window.Recharts && window.React && window.ReactDOM && window.ReactDOM.createRoot) {
        drawRecharts(el, cfg);
        // give React a couple frames to commit, then verify it painted
        setTimeout(function () {
          if (rechartsPainted(el)) return;               // success — Recharts is showing
          // failed to paint: tear down the empty root and restore the native chart
          if (el.__root) { try { el.__root.unmount(); } catch (e) {} el.__root = null; }
          drawNative(el, cfg);
        }, 120);
        return;
      }
      if (tries++ < 25) { setTimeout(attempt, 140); return; } // ~3.5s window for the CDN
      // Recharts never loaded — native chart already shown.
    };
    attempt();
  }
