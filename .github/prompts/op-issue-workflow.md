...
This playbook captures the ideal end-to-end workflow for taking an issue from intake to merged PR. Issue #313 showed that implicit workflow state is easy to lose, so the playbook keeps decisions and live `agent-state:v1` updates visible.

> **Important — never hardcode the canary.** Every phase that records the AGENTS.md handshake token or version MUST read the **current** value from `AGENTS.md` at the time the work happens.
...
3. Emit that exact token on its own line as the first substantive content of your reply. Do not paraphrase the number — copy it verbatim from the file you just read.
...
- **Exempt** if the issue is pure repo-process / ADR / doc-sync / canary-bump work (no end-user surface, no product shape). Record the rationale directly in the plan prose.
...
2. Architect returns a plan body conforming to [`.github/PLAN_TEMPLATE.md`](../PLAN_TEMPLATE.md), including applicable roles, dispatch order, ADR/doc-sync impact, risks, and exact verification commands.
   - **Plan quality self-checks.** Architect MUST satisfy each before posting (Mode B dogfood test, sandbox issue #18 May 2026, surfaced both as MAJOR diff-gate findings that mechanical plan-checks missed). Record the result as a one-line note in the plan prose.
...
**Canary lifecycle:** if your plan touches anything that hardcodes the AGENTS.md version, enumerate every affected file in the plan so they land in the same atomic change.
...
3. Each subagent returns either a verifiable commit and push or a concrete patch for the parent to apply. **Never trust a "push succeeded" claim without fetching to confirm** — runtime ghost-success is a known failure mode in some agent surfaces.
...
5. **Patch pass-back (parent-OP path).** If a subagent's runtime cannot persist edits, parent OP applies its concrete patch directly. The full ritual:
   ...
   7. Note the substitution in the next agent-state comment with the subagent role and resulting commit SHA.
...
**Default-branch-only workflow PRs (special case).** If your plan touches a `.github/workflows/*.yml` whose triggers fire only from the default branch ref — see the [Workflow verifiability matrix](../../docs/guides/agent-pipeline.md#workflow-verifiability-matrix) for the canonical list (`pull_request_review`, `pull_request_review_comment`, `pull_request_target`, `issue_comment`, `push`, `schedule`, `workflow_run`) — the in-repo `git fetch && git log` verification above is necessary but not sufficient. Use the sandbox sibling repo per [`docs/guides/sandbox-verification.md`](../../docs/guides/sandbox-verification.md), exercise the trigger end-to-end, and capture links in the PR body. ADR-016 is the durable rationale.
