...
function App() {
  ...
  function drillWeakest(categoryId: string): void {
    ...
    const domainQuestions = bank.questions.filter((q) => {
      const domainId = blueprint.structure === 'domain_task' ? String(q.domain) : (q.legacy_section ?? (q.task ? blueprint.crosswalk_from_new_task[q.task] : undefined));
      return domainId === categoryId && (settings.includeDrafts || q.status === 'reviewed');
    });
