...
**Status**: In Progress (validation + reference index in place; **505 draft items** across all blueprint tasks — ~500 draft bank target reached; shift focus to SME review)
...
- [x] **Batch 42** (`batch-42.json` in each domain): +12 draft items (`cctc-1166`–`1169`, `2165`–`2168`, `3165`–`3168`); mealtime/fecal-contact hand-hygiene, post-pet-contact hand hygiene, mealtime/household hygiene `complex_combo`, immunosuppression-duration and biologic/phototherapy skin-cancer risk, broad-brimmed-hat precautions, immunosuppression/biologic-risk and hat-prevention `complex_combo`, Aspergillus dyspnea/hypoxia and nodular-pneumonia imaging, voriconazole alternative therapy, Aspergillus evaluation/treatment `complex_combo`. Full `npm run validate` green (2026-06-05). **Bank milestone: 505 draft items.**
...
- [x] **Batch 37** (`batch-37.json` in each domain): +12 draft items (`cctc-1146`–`1149`, `2145`–`2148`, `3145`–`3148`); hepatitis A/B and earlier-booster vaccination, expectation-outcome psychological care, health-maintenance `complex_combo`, PTLD weight loss/fever/mono-like presentation, PTLD presentation `complex_combo`, wound granulation-tissue management, bacterial-pneumonia repositioning, CVL daily cultures, CVL-infection `complex_combo` (adult/both only). Full `npm run validate` green (2026-06-05).
...
- [x] Verification stubs — [`docs/reference/verification-stubs/README.md`](../docs/reference/verification-stubs/README.md), [`schema/reference-verification-stub.schema.json`](../schema/reference-verification-stub.schema.json), `questions/.verification/` (505 items as of batch 42).
